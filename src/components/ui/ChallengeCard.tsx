...
export const ChallengeCard = ({ title, desc, fix, image }: ChallengeCardProps) => (
  <div className="relative group h-[500px] overflow-hidden rounded-sm border border-white/10 hover:border-old-gold/50 transition-all duration-500">
    <Image src={image} alt={title} fill className="object-cover group-hover:scale-110 transition-transform duration-1000"/>
    ...
    <div className="absolute inset-0 p-10 flex flex-col justify-end z-20">
      <h4 className="text-3xl font-black uppercase mb-4 italic leading-none">{title}</h4>
      <p className="text-ecru-white/70 leading-relaxed mb-8 font-medium">{desc}</p>
